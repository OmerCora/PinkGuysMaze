...
std::string cParticleEmitter::GetObjectName(void)
{
	return this->objectName;
}

void cParticleEmitter::SetObjectName(std::string name)
{
	...
	for (int index = 0; index != this->m_vecParticles.size(); index++)
	{
		...
		{	// Update it as it's alive
			...
			if (this->particleSettings.isImposter)
			{
				...
				glm::vec4 eye;
				gelCameraManager::sharedInstance().mainCamera->GetEyePosition(eye);
				this->UpdateImposterEye(glm::vec3(eye.x, eye.y, eye.z));
			}
